...
import { a as __exportAll } from "./ssr.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/router-wTJ_phhm.js
...
var styles_default = "/assets/styles-PqAdhvFP.css";
var THEME_BOOT = `try{var t=JSON.parse(localStorage.getItem("mash-v1")||"{}");var th=(t.state&&t.state.theme)||"dark";var d=th==="system"?(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"):th;document.documentElement.dataset.theme=d}catch(e){document.documentElement.dataset.theme="dark"}`;
var Route$5 = createRootRoute({
	head: () => ({
		meta: [
			...
			{
				name: "description",
				content: "mash — fast answers, images, and code."
			},
			{
				name: "theme-color",
				content: "#0f0e10"
...
async function fill(id, input) {
	...
	const maxTokens = input.voice ? 80 : input.mode === "chat" ? 1200 : 4e3;
	try {
		const upstream = await fetch("https://api.x.ai/v1/chat/completions", {
			...
			body: JSON.stringify({
				model: "grok-4.5",
				stream: true,
				temperature: input.voice ? .3 : input.mode === "chat" ? .4 : .2,
				reasoning_effort: input.voice || input.mode === "chat" ? "low" : "medium",
				max_tokens: maxTokens,
				messages: input.messages
			})
		});
		if (!upstream.ok || !upstream.body) {
			job.error = `mash is busy (${upstream.status}). Try again.`;
			job.done = true;
			return;
		}
		...
		job.done = true;
		setTimeout(() => bucket().delete(id), 12e5);
	}
}
var chatHits = [];
function limited() {
	const now = Date.now();
	while (chatHits.length && now - chatHits[0] > 6e4) chatHits.shift();
	if (chatHits.length >= 18) return true;
	chatHits.push(now);
	return false;
}
function systemPrompt(mode, voice) {
	return [
		"You are mash. You solve the task they meant: a designed document, a site they can preview, a number, or a decision.",
		"Short when the question is small. Specific when they are paying for the answer.",
		voice ? "They are on a call. Reply in one short spoken sentence, the answer first. If it is a number, say the number. No markdown." : mode === "document" ? "Design the exact document they asked for. Inside one closed ```pdf fence, first line may be @theme navy or gold or forest or ink or rose or slate, then @kicker with a short label, then a # title, ## sections, - bullets, > a pull quote if it helps, and a markdown table wherever there are figures. Use their names, prices, and dates. No lorem. One sentence before the fence. Nothing after." : mode === "code" || mode === "build" ? "Build a finished, high-end page. Strong type, a real layout, mobile width, real copy, and states a customer can see. No purple gradient template, no TODOs, no lorem. End with one closed ```html fence that is a full document. One sentence before the fence. Nothing after. A PDF brief ends with one closed ```pdf fence instead, using @theme, @kicker, headings, and tables." : "Answer like a sharp person. The first line is the result. A simple calculation is the number, then one short line. Algebra or a proof is a few short steps: a plain title such as Step 2: Substitute, then one equation on its own line inside \\[ and \\]. A symbol inside a sentence uses \\( and \\). Never wrap a sentence in math and never print a raw backslash. For a probability or a trading idea, give the setup, the probability or expectancy, the assumption, the risk, and what would prove it wrong. Never promise a profit. Match their language, including Roman Urdu. Do not invent prices or citations."
	].join(" ");
}
function contentOf(message, withImage) {
	const text = message.content.slice(0, 8e3);
	const pics = [...message.images ?? [], ...message.image ? [message.image] : []].filter((url) => url.startsWith("data:image/") || url.startsWith("http")).slice(0, 20);
	if (!withImage || !pics.length) return text;
	return [{
		type: "text",
		text: text || "Look at these images."
	}, ...pics.map((url) => ({
		type: "image_url",
		image_url: { url: url.slice(0, 18e4) }
	}))];
}
var Route$2 = createFileRoute("/api/chat")({ server: { handlers: { POST: async ({ request }) => {
	if (!process.env.XAI_API_KEY) return Response.json({ error: "AI is not available right now." }, { status: 503 });
	if (limited()) return Response.json({ error: "Short pause after heavy use." }, { status: 429 });
	let body;
	try {
	...
	const mode = body.mode === "code" || body.mode === "build" || body.mode === "document" ? body.mode : "chat";
	const voice = body.voice === true;
	const messages = (Array.isArray(body.messages) ? body.messages.slice(-10) : []).filter((m) => m && (m.role === "user" || m.role === "assistant") && typeof m.content === "string").map((m, index, arr) => ({
		role: m.role,
	...
	const jobId = startChatJob({
		...
		messages: [{
			role: "system",
			content: systemPrompt(mode, voice)
		}, ...messages]
	});
	return Response.json({ id: jobId });
} } } });
var Route$1 = createFileRoute("/api/job")({ server: { handlers: { GET: async ({ request }) => {
	const job = readJob(new URL(request.url).searchParams.get("id") ?? "");
...
//#endregion
export { PACKAGE_VERSION as a, GENERIC_OAUTH_ERROR_CODES as i, GROK_PROVIDERS as n, getBaseURL as o, hasGateSessionMarker as r, router_exports as t };
